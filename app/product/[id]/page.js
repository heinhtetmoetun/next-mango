...
export default async function ProductDetail({ params }) {
  ...
  return (
    <main className="m-6 p-6 border rounded bg-white shadow space-y-4">
      ...
      <p><span className="font-semibold">Price:</span> {product.price?.toLocaleString()} Baht</p>
      <p><span className="font-semibold">Category:</span> {product.category ? product.category.name : "—"}</p>

      <div className="flex gap-4 pt-4">
        <Link
          href={`/product/${product._id}/edit`}   // ✅ no extra /fin-customer
        ...
        <Link
          href="/product"   // ✅ no extra /fin-customer
