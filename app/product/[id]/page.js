...
export default async function ProductDetail({ params }) {
  // On the server, NEXT_PUBLIC_API_BASE may be relative → fallback to absolute
  const base =
    process.env.NEXT_PUBLIC_API_BASE?.startsWith("http")
      ? process.env.NEXT_PUBLIC_API_BASE
      : "http://localhost:3001/fin-customer/api"; // 👈 absolute fallback

  const res = await fetch(`${base}/product/${params.id}`, {
    cache: "no-store",
  });
